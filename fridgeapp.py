...
#Header Frame - Chile of Notebook Frame--------------------------------------------------------------------------------------
        ...
        self.header1 = ttk.Label(self.frame_header, text = "Inventory", font = "Arial 30 bold", justify = 'center').grid(row = 0, column = 1, stick = 's')
        self.header2 = ttk.Label(self.frame_header, text = "Please fill out for every variety of beer!", font = "Arial 20 italic", justify = 'center').grid(row = 1, column = 1, sticky = N)
        self.logo = PhotoImage(file = "KappaSigma.gif").subsample(2,2)
        logo_image = ttk.Label(self.frame_header, image = self.logo).grid(row = 0, column = 0, rowspan = 2, padx = 10, pady = 10, sticky = E)
        self.frame_header.columnconfigure(1, weight = 1)

        self.begin_inventory = ttk.Button(self.frame_header, text = "Start Beer Inventory", command = self.start_inventory)
...
#Content Frame = Child of Notebook Frame-----------------------------------------------------------------------------------------------
        ...
        ttk.Label(self.frame_content, text = "UPC:", font = "Arial, 13").grid(row = 0, column = 0,  pady = 5, sticky = E)
        ttk.Label(self.frame_content, text = "Beer Name + Brand:", font = "Arial, 13").grid(row = 1, column = 0, pady = 5, sticky = E)
        ttk.Label(self.frame_content, text = 'Beer Type', font = 'Arial, 15').grid(row = 2, column = 0, sticky = E)
        ttk.Label(self.frame_content, text = "Quantity:", font = "Arial, 13").grid(row = 3, column = 0, pady = 5, sticky = E)
        ...
        self.kinds = [None, 'IPA', 'Stout', 'Porter','Pale Ale', 'Ale', 'Wheat', 'Amber', 'Lager', 'Pilsner', 'Barley Wine', 'Sour', 'Bock', 'Cider']
        self.entry_beerkind = ttk.OptionMenu(self.frame_content, self.beer_kind, *self.kinds)
...
#Button Frame - Child of Notebook Frame---------------------------------------------------------------------------------------------------
        ...
        self.submit = ttk.Button(self.frame_button, text = "Submit", command = self.confirmation).grid(row = 0, column = 0,padx = 10, pady = 10)
        self.clear = ttk.Button(self.frame_button, text = "Clear", command = self.clearAll).grid(row = 0, column = 1, padx = 10, pady = 10)
...
#Functions ---------------------------------------------------------------------------------------------------
    def buildFrame(self):
        ...
        #Beer Header
        Header1 = ttk.Label(self.frame_header2, text = "Beer Checkout", font = "Arial 26 bold", justify = 'center')
        Header1.grid(row = 0, column = 2, columnspan = 5, stick = S)

        #Please Scan Header
        Header2 = ttk.Label(self.frame_header2, text = "Please scan each beer you take!", font = "Arial 20 italic", justify = 'center')
        Header2.grid(row = 1, column = 2, columnspan = 5, sticky = N, padx=10)

        #Logo
        logo_image = ttk.Label(self.frame_header2, image = self.logo)
        logo_image.grid(row = 0, column = 0, rowspan = 3, padx = 10, pady = 10, sticky = N+E+W+S)
        ...
        #PIN Prompt
        self.scanPrompt = ttk.Label(self.frame_header2, text = "Please enter your pin: ", font = "Arial 22 bold", justify = 'center')
        ...
        self.initializeButton= ttk.Button(self.frame_header2, text = "Begin Scanning Beers", command = self.Initialize)
        self.initializeButton.grid(row = 4, column = 3, pady=30)

        self.exitPrompt = ttk.Label(self.frame_header2, text = "Press Delete or Enter to Exit/Restart: ", font = "Arial 12", justify = 'center')
    ...
    def submitBeer(self, event):
        ...
        if "failure" in response['result']:
            ...
            if self.upcString in self.checkedBeers:
                ...
                self.beer = ttk.Label(self.frame_header2, text = str(self.quantity[self.checkedBeers.index(self.upcString)]) + ": " + response['name'], font = "Arial 18", justify = 'center')

                if self.checkedBeers.index(self.upcString) <4:
                    self.beer.grid(row = self.checkedBeers.index(self.upcString)+3, column = 2)

                else:
                    self.beer.grid(row = self.checkedBeers.index(self.upcString)-1, column = 3)
    ...
    def keypress(self, event):
        ...
        x = event.keysym
        print (x)
        if x in ('KP_1', 'KP_2', 'KP_3', 'KP_4', 'KP_5', 'KP_6', 'KP_7', 'KP_8', 'KP_9', 'KP_0'):
            ...
            if self.current_digit < 4:
                ...
                nameLabel = ttk.Label(self.frame_header2, text = "Please scan your beers: " + response['initials'] + "!", font = "Arial 22 underline", justify = 'center')
                ...
                beerLabel = ttk.Label(self.frame_header2, text = "Beers: ", font = "Arial 22", justify = 'center')
                ...
                threading.Thread(target=lockSwitch).start()
    ...
    def confirmation(self):
        ...
        threading.Thread(target=meisterswitch).start()
        ...
        if self.BEER_ID is not None: #return beer id and quantity
           ...
           headers = {'UPC': str(self.UPC), 'NAME': str(self.Beer_Type), 'QUANTITY':int(self.Quantity), 'TYPE': str(self.beer_kind.get())}
           response = self.ping('add', headers)
        self.clearAll()
